...
export const getMyInformations = async (dispatchUser) => {
  ...
  try {
    const res = await axios.get("https://playvergrserver2-1.onrender.com/api/user/find/" + objectID, {
      headers: {
        Authorization:
...
export const updateMyInformations = async (id, user, dispatchUser) => {
  ...
  try {
    const res = await axios.put("https://playvergrserver2-1.onrender.com/api/user/" + id, user, {
      headers: {
        Authorization:
          "Bearer " + JSON.parse(localStorage.getItem("user")).token,
      },
...
export const addMovieToWatchedMovies = async (movieId, user, dispatchUser) => {
  dispatchUser(updateMyInformationsStart());
  try {
    const res = await axios.put("https://playvergrserver2-1.onrender.com/api/user/" + user._id + "/" + movieId, user, {
      headers: {
        Authorization:
          "Bearer " + JSON.parse(localStorage.getItem("user")).token,
      },
